...
class Statistics:
	...
	def __display_statistics(self, database_type: str, dataset: str, action: str):
		print(
			self.__execution_times[
	...
	def display_statistics(self) -> None:
		...
		for database_type in ['CouchDB', 'MongoDB', 'Redis']:
			for dataset in sorted(os.listdir(os.path.join(os.getcwd(), 'data', 'json_data'))):
				for action in ['insert', 'read', 'update', 'delete']:
	...
	def __export_plot(self, action:str) -> None:
		...
		fig.update_layout(
			title=f'NoSQL Databases - {action.capitalize()} ({self.__iterations} Iterations)',
		...
		html_export_path = os.path.join(self.__export_folder_path, 'plots', 'html')
		png_export_path = os.path.join(self.__export_folder_path, 'plots', 'png')

		if not os.path.exists(html_export_path):
			os.mkdir(html_export_path)

		if not os.path.exists(png_export_path):
			os.mkdir(png_export_path)

		fig.write_html(os.path.join(html_export_path,f'{filename}_{self.__iterations}.html'))
		fig.write_image(os.path.join(png_export_path,f'{filename}_{self.__iterations}.png'))
